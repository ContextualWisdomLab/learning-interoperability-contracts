...
| JSON Schema validity | Quality workflow pins `jsonschema==4.26.0`, runs `Draft202012Validator.check_schema`, and includes a negative metaschema regression | Candidate validation on active bootstrap head | Exact head `946d71897306eba84be384078898ef788baf0140` has quality pending and security/SAST queued as of 2026-09-01; therefore no current-head success receipt exists | Require successful exact-head quality/security/SAST checks and then record the receipt rather than predecessor evidence |
| Timestamp contract | Range-constrained lexical pattern plus `Draft202012Validator(..., format_checker=FormatChecker())`, parser-backed calendar checks, and positive/negative edge cases | Candidate validation; exact-head evidence pending | Contract rejection now covers invalid calendar dates and annotation-only `format` risk in the repository gate, but the current head has not completed CI and consumers have no cross-language conformance proof | Require current-head quality success, then add consumer contract fixtures in issue #3/generated SDK work |
| Standards portfolio | `docs/doctoring/STANDARD_TRACEABILITY.md`, AGENTS.md | Adoption decisions recorded; conformance not evidenced | Adoption and implementation evidence are now separated; requirement-level executable evidence is still absent for unimplemented surfaces and QTI Metadata still needs a requirement-level source before implementation | Pin exact normative requirements/test paths as each surface becomes executable; never promote adoption to conformance without exact-head evidence |
...
| Operability/security | Security/SAST workflows exist | Partial | Current-head security/SAST evidence is queued; no runtime service belongs here | Keep repository read-only at runtime, validate supply-chain dependencies and generated artifacts in CI |
